...
@ViewModelScoped
class LinkXAccountUseCase @Inject constructor(
    private val linkXAccountRepository: LinkXAccountRepository
) {
    operator fun invoke(
        linkXAccountTask: Task<AuthResult>
    ): Flow<Result<Unit, LinkXAccountError>> = flow {
        return@flow emit(linkXAccountRepository.linkXAccount(linkXAccountTask = linkXAccountTask))
    }
}